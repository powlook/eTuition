# eTuition Portal — Build Rules

These rules apply to every agent working on this project.

## The job

Build eTuition Portal exactly as specified in [requirements.md](./requirements.md). That document
is the contract: its features, specifications, and success criteria decide when work is done. When in
doubt, requirements.md wins.

## The team

- **orchestrator** (primary) — plans, delegates, reviews, gates phases. Does not write code.
- **frontend-dev** — all frontend UI, student dashboard, math formula rendering, and unit tests.
- **backend-dev** — all backend APIs, database models (user access management, curriculum topics), automated problem & working generation engine, and unit tests.
