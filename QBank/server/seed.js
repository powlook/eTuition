...
import { exportQuestionsToExcel } from './excelService.js';
...
const ITEMS = ['apples', 'pencils', 'books', 'candies', 'marbles', 'stickers', 'toys', 'stamps', 'notebooks', 'crayons'];
...
// Load authentic DepEd MATATAG Curriculum extracted from MATATAG-Curriculum-Grade-1-10-Maths.xlsx (Both Grade 1-6 & Grade 7-10 sheets)
const topicsJsonPath = path.join(__dirname, '..', 'matatag_topics.json');
const depEdMatatagCurriculum = JSON.parse(fs.readFileSync(topicsJsonPath, 'utf8'));
...
console.log(`🚀 Seeding ALL ${depEdMatatagCurriculum.length} DepEd MATATAG Curriculum Topics (Form 1 to Form 10) from MATATAG-Curriculum-Grade-1-10-Maths.xlsx...`);
...
console.log(`✅ Seeded ALL ${depEdMatatagCurriculum.length} authentic DepEd MATATAG Topics into SQLite database.`);

console.log(`🚀 Generating 5,280 Topic-Matched Questions (30 per topic across all 176 topics)...`);
...
console.log(`🎉 Seeded ${totalQuestions} authentic MATATAG questions into SQLite database!`);

console.log('🚀 Syncing and exporting full question bank to questions_bank.xlsx...');
const excelPath = path.join(__dirname, '..', 'questions_bank.xlsx');
exportQuestionsToExcel(db, excelPath);

console.log('✨ Seed and Excel Export process complete!');
