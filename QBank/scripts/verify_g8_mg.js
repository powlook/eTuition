...
export function verifyGrade8MeasurementGeometry() {
  console.log('--- VERIFICATION REPORT: GRADE 8 MEASUREMENT & GEOMETRY ---');
  ...
  console.log('----------------------------------------------------');
  console.log(`Total Grade 8 Measurement & Geometry Questions verified: ${totalVerified}`);
  console.log(`Missing SVG Plot Files: ${missingImages}`);
  console.log(`Total Validation Errors: ${totalErrors}`);

  if (totalErrors === 0) {
    console.log('🎉 ALL 150 QUESTIONS & SVG PLOTS VERIFIED 100% CLEAN!');
  } else {
...
if (process.argv[1] && process.argv[1].endsWith('verify_g8_mg.js')) {
  verifyGrade8MeasurementGeometry();
}
