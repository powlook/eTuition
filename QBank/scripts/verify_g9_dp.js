...
export function verifyGrade9DataProbability() {
  console.log('--- VERIFICATION REPORT: GRADE 9 DATA & PROBABILITY ---');
  ...
  for (const topicId of topicIds) {
    ...
    questions.forEach((q, idx) => {
      ...
      // Validate SVG Image file existence on disk & XML validity
      if (!q.image_url) {
        ...
        if (!fs.existsSync(diskPath)) {
          ...
          // Check for unescaped < inside text
          if (content.includes(' (a < 0)') || content.includes(' < ') || content.includes('< 0')) {
            console.error(`❌ ERROR in SVG XML Q ID ${q.id}: Contains unescaped < in text tag`);
            totalErrors++;
          }
        }
      }
  ...
  console.log('----------------------------------------------------');
  console.log(`Total Grade 9 Data & Probability Questions verified: ${totalVerified}`);
  console.log(`Missing SVG Plot Files: ${missingImages}`);
  console.log(`Total Validation Errors: ${totalErrors}`);

  if (totalErrors === 0) {
    console.log('🎉 ALL 100 QUESTIONS & SVG PLOTS VERIFIED 100% CLEAN!');
  } else {
...
if (process.argv[1] && process.argv[1].endsWith('verify_g9_dp.js')) {
  verifyGrade9DataProbability();
}
