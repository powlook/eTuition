...
console.log('====================================================');
console.log('🔍 VERIFYING GRADE 10 MEASUREMENT & GEOMETRY QUESTIONS');
console.log('====================================================\n');

// 1. Verify SQLite Database
const topicIds = [161, 162, 163, 164];
...
if (fs.existsSync(qbPath)) {
  ...
  rows.forEach(r => {
    ...
    if ((formLvl === '10' || formLvl === 'Form 10') && strand.includes('Measurement')) {
      qbForm10MgCount++;
    }
  });

  console.log(`📊 questions_bank.xlsx Form 10 MG records: ${qbForm10MgCount}`);
  if (qbForm10MgCount !== 200) {
    console.error(`❌ ERROR: questions_bank.xlsx should contain 200 Form 10 MG questions, found ${qbForm10MgCount}`);
    dbErrorCount++;
  }
} else {
...
let g10MgSheetCount = 0;

if (fs.existsSync(g10Path)) {
  const wb = xlsx.readFile(g10Path);
  const sheetName = 'Measurement & Geometry (MG)' in wb.Sheets ? 'Measurement & Geometry (MG)' : 'Measurement and Geometry (MG)';
  ...
  console.log(`📄 Grade_10_Math_Questions.xlsx sheet "${sheetName}" records: ${g10MgSheetCount}`);
  if (g10MgSheetCount !== 200) {
    console.error(`❌ ERROR: Grade_10_Math_Questions.xlsx should contain 200 items in MG sheet, found ${g10MgSheetCount}`);
    dbErrorCount++;
  }
} else {
