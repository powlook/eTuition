...
export function verifyGrade9NumberAlgebra() {
  console.log('--- VERIFICATION REPORT: GRADE 9 NUMBER & ALGEBRA ---');
  ...
  console.log('----------------------------------------------------');
  console.log(`Total Grade 9 Number & Algebra Questions verified: ${totalVerified}`);
  console.log(`Missing SVG Plot Files: ${missingImages}`);
  console.log(`Total Validation Errors: ${totalErrors}`);

  if (totalErrors === 0) {
    console.log('🎉 ALL 250 QUESTIONS & SVG PLOTS VERIFIED 100% CLEAN!');
  } else {
...
if (process.argv[1] && process.argv[1].endsWith('verify_g9_na.js')) {
  verifyGrade9NumberAlgebra();
}
