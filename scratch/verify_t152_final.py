...
for db_path in dbs:
    ...
    c = conn.cursor()
    c.execute("SELECT id, question_text, options_json, correct_answer, show_image, image_url FROM questions WHERE topic_id = 'T152' OR topic_id = '152'")
    rows = c.fetchall()
    report.append(f"DB {db_path}: {len(rows)} questions found")
    generic_count = 0
    for qid, qtxt, opts_str, ans, simg, imgurl in rows:
        opts = json.loads(opts_str)
        if 'The sum of all three sides equals 180' in opts or 'The product of two sides equals the third side' in opts:
            generic_count += 1
...
for jpath in jsons:
    with open(jpath, 'r', encoding='utf-8') as f:
        data = json.load(f)
    t152 = [q for q in data if q.get('topic_id') == 'T152' or q.get('topic_id') == 152]
    report.append(f"JSON {jpath}: {len(t152)} questions found")
    generic_count = sum(1 for q in t152 if 'The sum of all three sides equals 180' in (q.get('options') or []) or 'The product of two sides equals the third side' in (q.get('options') or []))
    report.append(f"  Generic placeholder options count in {jpath}: {generic_count}")

# Check specific questions Q134484 and Q134502
conn = sqlite3.connect('server/etuition.db')
c = conn.cursor()
for check_id in [134484, 134502]:
    ...
    row = c.fetchone()
    report.append(f"\n--- Question ID {check_id} ([T152-Q{check_id}]) ---")
...
output_text = "\n".join(report)
with open("scratch/t152_final_report.txt", "w", encoding="utf-8") as f:
    f.write(output_text)

print("Saved scratch/t152_final_report.txt successfully")
