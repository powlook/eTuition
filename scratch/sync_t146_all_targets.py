...
print(f"Loaded {len(built_qs)} built questions for Topic 146.")

# Get starting ID for Topic 146 from server/etuition.db
conn_main = sqlite3.connect('server/etuition.db')
cursor_main = conn_main.cursor()
cursor_main.execute("SELECT id FROM questions WHERE topic_id = 146 ORDER BY id")
existing_ids = [r[0] for r in cursor_main.fetchall()]
print(f"Existing IDs for Topic 146 in server/etuition.db: {len(existing_ids)} (e.g., {existing_ids[:5]})")

# Map 50 questions to existing IDs or create new ones
full_records = []
for idx, bq in enumerate(built_qs):
    q_id = existing_ids[idx] if idx < len(existing_ids) else (134183 + idx)
    img_url = f"/images/g9_t146_q{bq['num']}.svg"
...
for db_path in dbs:
    if os.path.exists(db_path):
        ...
        # Delete existing topic 146 rows
        cursor.execute("DELETE FROM questions WHERE topic_id = 146")
        ...
        for r in full_records:
            ...
            """, (
                ...
                r['image_url'], r['image_alt'], r['difficulty']
            ))
        conn.commit()
        conn.close()
...
for jp in json_paths:
    if os.path.exists(jp):
        ...
        data = [q for q in data if str(q.get('topic_id')) != '146']
        for r in full_records:
            data.append(r)
