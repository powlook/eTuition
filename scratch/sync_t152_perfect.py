...
with open('scratch/t152_50_perfect_built.json', 'r', encoding='utf-8') as f:
    t152_data = json.load(f)

print(f"Loaded {len(t152_data)} questions from scratch/t152_50_perfect_built.json")

# Map of question id to dict
t152_map = {q['id']: q for q in t152_data}
...
for jpath in jsons:
    ...
    for item in data:
        ...
        num_id = item_id_str.replace('T152-Q', '').replace('Q', '')
        if num_id in t152_map:
            q = t152_map[num_id]
            opts = [q['optA'], q['optB'], q['optC'], q['optD']]
...
if os.path.exists(excel_path):
    ...
    for row in range(2, ws.max_row + 1):
        ...
        num_id = q_id_val.replace('T152-Q', '').replace('Q', '')
        if num_id in t152_map:
            q = t152_map[num_id]
...
print("T152 synchronization complete!")
