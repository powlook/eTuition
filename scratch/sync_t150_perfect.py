...
with open('scratch/t150_50_perfect_built.json', 'r', encoding='utf-8') as f:
    t150_data = json.load(f)

print(f"Loaded {len(t150_data)} questions from scratch/t150_50_perfect_built.json")

# Map of question id to dict
t150_map = {q['id']: q for q in t150_data}
...
for jpath in jsons:
    ...
    for item in data:
        ...
        num_id = item_id_str.replace('T150-Q', '').replace('Q', '')
        if num_id in t150_map:
            q = t150_map[num_id]
            opts = [q['optA'], q['optB'], q['optC'], q['optD']]
...
if os.path.exists(excel_path):
    ...
    for row in range(2, ws.max_row + 1):
        ...
        num_id = q_id_val.replace('T150-Q', '').replace('Q', '')
        if num_id in t150_map:
            q = t150_map[num_id]
...
print("T150 synchronization complete!")
