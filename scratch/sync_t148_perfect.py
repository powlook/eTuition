...
with open('scratch/t148_50_perfect_built.json', 'r', encoding='utf-8') as f:
    t148_data = json.load(f)

print(f"Loaded {len(t148_data)} questions from scratch/t148_50_perfect_built.json")

# Map of question id to dict
t148_map = {q['id']: q for q in t148_data}
...
for jpath in jsons:
    ...
    for item in data:
        ...
        # extract numeric id if formatted like T148-Q134283
        num_id = item_id_str.replace('T148-Q', '').replace('Q', '')
        if num_id in t148_map:
            q = t148_map[num_id]
            opts = [q['optA'], q['optB'], q['optC'], q['optD']]
...
if os.path.exists(excel_path):
    ...
    for row in range(2, ws.max_row + 1):
        ...
        num_id = q_id_val.replace('T148-Q', '').replace('Q', '')
        if num_id in t148_map:
            q = t148_map[num_id]
...
print("T148 synchronization complete!")
