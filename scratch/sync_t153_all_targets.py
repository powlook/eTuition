...
print(f"Loaded {len(built_qs)} built questions for Topic 153.")

# Get existing IDs for Topic 153 from server/etuition.db
conn_main = sqlite3.connect('server/etuition.db')
cursor_main = conn_main.cursor()
cursor_main.execute("SELECT id FROM questions WHERE topic_id = 153 ORDER BY id")
existing_ids = [r[0] for r in cursor_main.fetchall()]
conn_main.close()

print(f"Existing IDs for Topic 153 in server/etuition.db: {len(existing_ids)} (e.g., {existing_ids[:5]})")

# Map 50 questions to records
full_records = []
for idx, bq in enumerate(built_qs):
    q_id = existing_ids[idx] if idx < len(existing_ids) else (134533 + idx)
    img_url = bq['img_url']
    show_img = 1 if bq['needs_img'] else 0
    ...
    record = {
        ...
        'image_url': img_url,
        'image_alt': f"Question {bq['num']} Diagram" if img_url else None,
        'difficulty': 2,
        'show_image': show_img,
        'show_formula': 1 if bq['formula'] else 0
    }
...
for db_path in dbs:
    if os.path.exists(db_path):
        ...
        cursor.execute("DELETE FROM questions WHERE topic_id = 153")
...
for jp in json_paths:
    if os.path.exists(jp):
        ...
        data = [q for q in data if str(q.get('topic_id')) != '153']
        for r in full_records:
            data.append(r)
...
print("Synchronization for Topic 153 complete across all DBs, JSONs, and Excel!")
