...
c.execute("""
    ...
    FROM questions
    WHERE topic_id = 137 OR question_title LIKE '%Pythagorean%'
    ORDER BY id ASC
""")
rows = c.fetchall()
conn.close()

print(f"Fetched {len(rows)} topic 137 questions from primary DB.")
...
print("Topic 137 synchronization complete!")
